...
def main():
  ...
  if args.VERBOSE:
    print('loading predicted and actual reference locations', file=sys.stderr)
  ...
  if args.VERBOSE:
    print('computing fragment mapping accuracy summary stats', file=sys.stderr)
  nFalsePositives = 0
  nFalseNegatives = 0
  nTruePositives = 0
  ...
  if args.VERBOSE:
    print('loading/extracting ref bases for predicted/actual', file=sys.stderr)
  predictedRefBases = loadPredictedRefBases(args.samFileName)
  actualRefBases = getActualRefBases(actualRefLocations)

  if args.VERBOSE:
    print('computing accuracy of mapping per simulated base', file=sys.stderr)
