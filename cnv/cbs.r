...
cbs.segment01 <- function(indir, outdir, varbin.gc, bad.bins.file, varbin.data, sample.name, alt.sample.name, alpha, nperm, undo.SD, min.width) {
	...
	write.table(thisShort, sep="\t", file=paste(outdir, "/", sample.name, ".hg19.5k.varbin.short.cbs.txt", sep=""), quote=F, row.names=F) 
	...
	chr.text <- c(1:22, "X", "Y")
	vlines.shift <- c(vlines[-1], 5*10^9)
  ...
  y.labels <- c("0.005", "0.020", "0.100", "0.5", "1", "2", "10", "100")
	...
	pdf(paste(outdir, "/", sample.name, ".5k.wg.pdf", sep=""), height=3.5, width=7, useDingbats=FALSE)
  par(pin=c(5.00,1.75))
	plot(x=thisRatio$abspos, y=thisRatio$lowratio, log="y", main=paste(sample.name, alt.sample.name), xaxt="n", xlab="Chromosomal position", yaxt="n", ylab="Ratio", col="#517FFF", cex=0.01)
  ...
  axis(2, at=y.at, labels=y.labels)
	...
	lines(x=thisRatio$abspos, y=thisRatio$seg.mean.LOWESS, col="red", cex=1.0)
	# abline(h=hlines)
	# abline(v=vlines)
  abline(v=vlines, lwd=0.1, col="grey")
	mtext(chr.text, at = chr.at, cex=0.4, side=1)
	dev.off()
	...
	# dev.off()
	# write.table(thisQuantalStats, sep="\t", file=paste(outdir, "/", sample.name, ".hg19.5k.varbin.quantal.stats.txt", sep=""), quote=F, row.names=F) 

	write.table(thisRatio, sep="\t", file=paste(outdir, "/", sample.name, ".hg19.5k.varbin.data.txt", sep=""), quote=F, row.names=F) 
	write.table(thisShort, sep="\t", file=paste(outdir, "/", sample.name, ".hg19.5k.varbin.short.txt", sep=""), quote=F, row.names=F) 
	...
	write.table(thisShort, sep="\t", file=paste(outdir, "/", sample.name, ".hg19.5k.nobad.varbin.short.cbs.txt", sep=""), quote=F, row.names=F) 
  ...
  y.labels <- c("0.005", "0.020", "0.100", "0.5", "1", "2", "10", "100")
	...
	pdf(paste(outdir, "/", sample.name, ".5k.wg.nobad.pdf", sep=""), height=3.5, width=7, useDingbats=FALSE)
  par(pin=c(5.00,1.75))
	plot(x=thisRatioNobig$abspos, y=thisRatioNobig$lowratio, log="y", main=paste(sample.name, alt.sample.name), xaxt="n", xlab="Chromosomal position", yaxt="n", ylab="Ratio", col="#517FFF", cex=0.01)
	# axis(1, at=x.at, labels=x.labels)
  axis(2, at=y.at, labels=y.labels)
	# lines(x=thisRatioNobig$abspos, y=thisRatioNobig$lowratio, col="#CCCCCC")
	# points(x=thisRatioNobig$abspos, y=thisRatioNobig$seg.mean.LOWESS, col="#0000AA")
  ...
  abline(v=vlines, lwd=0.1, col="grey")
	...
	# png(paste(outdir, "/", sample.name, ".5k.wg.nobad.quantal.png", sep=""), height=800, width=1200)
	...
	# mtext(chr.text, at = chr.at)
	# dev.off()
	# write.table(thisQuantalStatsNobad, sep="\t", file=paste(outdir, "/", sample.name, ".hg19.5k.nobad.varbin.quantal.stats.txt", sep=""), quote=F, row.names=F) 
